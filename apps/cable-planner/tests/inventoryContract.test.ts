...
// Suite-Overlay: upstream liest `src/renderer/types/inventory.ts`. Hier ist
// genau diese Datei durch @avplan/inventory-core ersetzt (siehe
// REPLACED_BY_PACKAGE in scripts/planner-drift.mjs), also liest der Guard
// den Interface-Rumpf aus dem Paket — dieselbe Zusage, eine Quelle weiter.
import inventoryTypesSrc from '../../../packages/inventory-core/src/types.ts?raw'
import {
  INVENTORY_FORMAT,
...
import type { InventoryItem, StorageNode, InventorySet, InventoryUnit } from '@avplan/inventory-core'
...
describe('avplan-inventory Wire-Contract (Drift-Guard)', () => {
  ...
  it('Envelope-Shape ist eingefroren', () => {
    const file = JSON.parse(serializeInventory(snapshot, { exportedAt: 't', app: 'cable-planner' }))
