...
describe('Der Lager-Vertrag ist die einzige Tuer (ADR-006)', () => {
  ...
  it('das Lager ist nicht leer und traegt seine Rechenschicht', () => {
    ...
    expect(drin.length).toBeGreaterThanOrEqual(24)
    expect(drin).toContain('lager/lib/inventoryCoverage.ts')
    expect(drin).toContain('lager/store/inventoryStore.ts')
    // OVERLAY (Suite): `lager/types/inventory.ts` gibt es hier nicht — die
    // Lager-Typen kommen aus `@avplan/inventory-core` (REPLACED_BY_PACKAGE in
    // scripts/planner-drift.mjs). Statt der Datei wird deshalb geprueft, dass
    // die Tuer sie aus dem Paket bezieht: dieselbe Zusage, eine Quelle weiter.
    expect(readFileSync(join(WURZEL, TUER), 'utf8')).toContain('@avplan/inventory-core')
