...
const LexwareCard = () => {
  ...
  useEffect(() => {
    lexware?.hasApiKey().then((stored) => {
      ...
      setStatus(
        stored
          ? t('settings.integrations.lexware.statusLoaded', 'Key on file')
          : t('settings.integrations.lexware.statusNone', 'No key on file'),
      )
    })
  ...
  const saveKey = async () => {
    ...
    try {
      ...
      setStatus(
        stored
          ? t('settings.integrations.lexware.statusSaved', 'Key stored securely.')
          : t('settings.integrations.lexware.statusNone', 'No key on file'),
      )
    } catch (error) {
      setStatus(
        error instanceof Error
          ? error.message
          : t('settings.integrations.lexware.saveFailed', 'Could not store the key'),
      )
    } finally {
      setBusy(false)
    }
  ...
  const testConnection = async () => {
    ...
    try {
      ...
      setStatus(
        result.ok
          ? t('settings.integrations.lexware.testOk', 'Connection ok')
          : result.error ?? t('settings.integrations.lexware.testFailed', 'Connection failed'),
      )
    } finally {
      setBusy(false)
    }
  ...
  return (
    <SettingsCard
      ...
      description={t(
        ...
        'API key from your Lexware Office account. Stored encrypted in the operating system keychain (never in the project file).',
      )}
    >
      <label className="block text-cp-base">
      ...
      >
        ...
        <div className="text-cp-text-faint">
          {t('settings.integrations.lexware.keyStored', 'Key stored:')}{' '}
          {hasKey ? t('common.yes', 'Yes') : t('common.no', 'No')}
        </div>
      </div>
      <div className="mt-2 flex flex-wrap gap-2">
        <button
          ...
          className="bg-sky-600 px-3 py-1 text-cp-base hover:bg-sky-500 disabled:opacity-50"
        >
          {t('settings.integrations.lexware.save', 'Save')}
        </button>
        <button
          type="button"
          ...
          className="bg-emerald-600 px-3 py-1 text-cp-base hover:bg-emerald-500 disabled:opacity-50"
        >
          {t('settings.integrations.lexware.test', 'Test connection')}
        </button>
      </div>
      <div className="mt-2 text-cp-xs text-cp-text-muted">
        {t(
