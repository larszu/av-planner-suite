...
export default function Header({
  ...
  //
  // IM SUITE-BAU EIN EIGENER DIALOG und nicht `window.prompt`: die Shell
  // fuehrt eigene Dialoge (ADR-007 Abschnitt 6 — Flaechen, kein Radius,
  // Aktionen unten rechts), und ein natives Fenster mitten darin sieht aus
  // wie ein Fremdkoerper. `scripts/native-dialogs.mjs` misst das.
  const handleSaveAs = useCallback(async () => {
    const vorschlag = defaultProjectFileName(buildProjectFile(useStore.getState()));
    const name = await promptDialog(t('header.saveAs.prompt', 'File name'), { defaultValue: vorschlag });
    if (!name) return;
    saveProject(name);
  }, [saveProject, t]);
    ...
    <>
      ...
      <header className="bc-topbar">
        <div className="flex min-w-0 shrink-0 items-center gap-2 text-bc-text-bright">
          {/* Marke — die Suite-Shell setzt ihren eigenen App-Titel darueber,
              hier waere er ein zweites Mal dasselbe. */}
          {!isEmbedded && (
            <>
            ...
            <>
              <MenuItem
                ...
                onClick={() => {
                  ...
                  //
                  // `destructive: true`: hier geht Arbeit verloren, und der
                  // rote Knopf sagt das — den kann ein natives Fenster nicht.
                  void confirmDialog(t('header.new.confirm', 'New project — the current one is replaced. Continue?'), {
                    destructive: true,
                  }).then((ja) => {
                    if (ja) newProject();
                  });
              ...
              {/* EINGEBETTET STELLT DIE SHELL DIESE DREI. Ein zweiter Weg
                  daneben waere nicht nur doppelt — er schriebe in eine andere
                  Ablage als die, aus der die Shell spaeter liest. Das „Neue
                  Projekt" darueber bleibt: es setzt DIESES Modul zurueck und
                  hat in der Shell keine Entsprechung. */}
              {!isEmbedded && (
                <>
