...
const VersionDialog: React.FC<Props> = ({ projectId, projectName, currentDoc, onRestore, onClose }) => {
  ...
  const uebernehmen = async () => {
    ...
    const ok = await confirmDialog(
      t('merge.confirm', '{n} von {total} Einträgen übernehmen? Alles ohne Wahl bleibt, wie es hier ist.')
          ...
          <div className="ver-list">
            ...
            {versions.length === 0 ? (
              ...
              <div key={v.id} className={`ver-row ${v.id === selectedId ? 'on' : ''}`} onClick={() => setSelectedId(v.id)}>
                ...
                <button className="ver-act" title={t('version.restore', 'Diesen Stand laden')} onClick={(e) => { e.stopPropagation(); void restore(v); }}><Icon name="open" size={15} /></button>
