...
const VersionDialog: React.FC<Props> = ({ projectId, projectName, currentDoc, onRestore, onClose }) => {
  ...
  // SUITE-OVERLAY (nicht upstream): `@avplan/ui` statt der nativen Dialoge.
  // Der Guard `dialogs:native` verbietet `window.confirm`/`window.alert` in
  // der Suite, und zwar mit Grund -- der native Dialog kann den roten Knopf
  // nicht, und hier haengt ein Verwerfen nicht gesicherter Arbeit dahinter.
  // Der Versionsname steht deshalb im `body` und nicht im Fragetext; der
  // Platzhalter {label} der Upstream-Fassung entfaellt dadurch.
  const restore = async (v: ProjectVersion) => {
    const ok = await confirmDialog(
      t('version.restoreConfirm', 'Load this version? Unsaved changes will be lost.'),
  ...
  // SUITE-OVERLAY: `confirmDialog` statt `window.confirm` — derselbe Grund wie
  // oben bei `restore`, und derselbe Guard (`dialogs:native`).
  const uebernehmen = async () => {
    if (!plan || !theirDoc) return;
    const entschieden = plan.entries.filter((e) => e.choice);
    if (entschieden.length === 0) return;
    const ok = await confirmDialog(
      t('merge.confirm', 'Apply {n} of {total} entries? Anything without a choice stays as it is here.')
        .replace('{n}', String(entschieden.length))
        .replace('{total}', String(plan.entries.length)),
    );
    if (ok) onRestore(applyMerge(currentDoc, theirDoc, plan.entries));
  };
          ...
          <div className="ver-list">
            ...
            {versions.length === 0 ? (
              ...
              <div key={v.id} className={`ver-row ${v.id === selectedId ? 'on' : ''}`} onClick={() => setSelectedId(v.id)}>
                ...
                <button className="ver-act" title={t('version.restore', 'Load this version')} onClick={(e) => { e.stopPropagation(); void restore(v); }}><Icon name="open" size={15} /></button>
