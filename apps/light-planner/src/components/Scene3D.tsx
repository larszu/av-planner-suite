...
const Scene3D = forwardRef<Scene3DHandle, Props>(({ fixtures, persons, stageElements, trusses, walls, ceilings, floorPlan, layers, cameras, selectedIds, showHeatMap, heatMapScale, heatMapTarget, photoMode, exposure, haze, showBeams, ambience, floor, sun, onSelect, onHoverLux }, ref) => {
  ...
  return (
    // Die Suite-Kopie hat einen eigenen Rahmen (`.scene3d-wrap`), der bereits
    // `position: relative` traegt; die Overlays haengen deshalb dort hinein und
    // die Zeichenflaeche bleibt `.scene3d-container`. Upstream gibt es den
    // Rahmen nicht, dort heisst die Flaeche `.scene3d-surface`.
