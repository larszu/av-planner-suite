      ...
      <div className="property-panel">
        ...
        <div className="prop-section">
          <span className="prop-section-title">
            ...
            <button type="button" className="beam-help-toggle" onClick={() => setBeamHelp((v) => !v)} title={t('prop.whatDoesThisMean', 'What does this mean?')}><Icon name="info" size={12} /></button>
          </span>
        ...
        <div className="prop-section">
          <span className="prop-section-title">
            ...
            <button type="button" className="beam-help-toggle" onClick={() => setBarnHelp((v) => !v)} title={t('prop.explainDifferences', 'Explain the differences')}><Icon name="info" size={12} /></button>
          </span>
          {(() => {
  ...
  if (selShape) {
    ...
    return (
      <div className="property-panel">
        ...
        {isRect && (
          <button className="auto-btn wide" onClick={onAreaLight}><Icon name="beam" size={13} />{t('prop.lightArea', 'Light this area')}</button>
        )}
    ...
    return (
      <div className="property-panel">
        ...
        <div className="prop-section">
          ...
          {multiFixtures.length > 0 && (
            <div className="reflectance-presets">
              <button className="refl-btn" onClick={() => { for (const mf of multiFixtures) onUpdateFixture(mf.id, { hidden: true }); }}><Icon name="eyeOff" size={13} />{t('prop.multiHide', 'Hide')}</button>
              <button className="refl-btn" onClick={() => { for (const mf of multiFixtures) onUpdateFixture(mf.id, { hidden: undefined }); }}><Icon name="eye" size={13} />{t('prop.multiShow', 'Show')}</button>
            </div>
          )}
        </div>
    ...
    <div className="property-panel">
      ...
      <div className="prop-section">
        ...
        <ol className="quickstart-list">
          <li><span><Icon name="plan2d" size={13} /></span> <strong>{t('prop.qs1strong', 'Floor plan')}</strong> {t('prop.qs1rest', 'import – JPG, PNG or PDF')}</li>
          <li><span><Icon name="ruler" size={13} /></span> <strong>{t('prop.qs2strong', 'Calibrate scale')}</strong> {t('prop.qs2rest', '– drag a segment, enter the real length')}</li>
