    ...
    <header className="topbar" ref={ref}>
      ...
      <div className="topbar-left">
        ...
        <div className="tb-menuwrap">
          <button className={`tb-icon ${open === 'menu' ? 'on' : ''}`} title={t('topbar.menuBtn', 'Menü')}
          ...
          {open === 'menu' && (
            <div className="tb-dropdown">
              ...
              <button className="tb-dd-item" onClick={run(p.onLoadFromFile)}><Icon name="import" size={15} />{t('menu.loadFile', 'Projekt aus Datei…')}</button>
              <div className="tb-dd-div" />
              <button className="tb-dd-item" onClick={run(p.onExportAvplan)} title={t('topbar.exportAvplanTitle', 'Gesamtprojekt (Raum + Licht + Kameras + Verkabelung) verlustfrei exportieren — von allen drei Apps lesbar, fremde Daten bleiben erhalten')}><Icon name="export" size={15} />{t('topbar.exportAvplan', 'Gesamtprojekt exportieren (.avplan)…')}</button>
              <button className="tb-dd-item" onClick={run(p.onImportAvplan)} title={t('topbar.importAvplanTitle', 'Gesamtprojekt (.avplan) importieren — Licht wird bearbeitbar geladen, Kamera-/Verkabelungs-Daten bleiben verlustfrei erhalten')}><Icon name="import" size={15} />{t('topbar.importAvplan', 'Gesamtprojekt importieren (.avplan)…')}</button>
              <div className="tb-dd-div" />
              <button className="tb-dd-item" onClick={run(p.onExportVenue)} title={t('topbar.exportVenueTitle', 'Geteilten Raum (Wände, Bühne, Personen, Floor-Plan) exportieren — importierbar im MultiCam-Planner')}><Icon name="export" size={15} />{t('topbar.exportVenue', 'Venue exportieren (.venue.json)…')}</button>
              <button className="tb-dd-item" onClick={run(p.onImportVenue)} title={t('topbar.importVenueTitle', 'Geteilten Raum importieren — ersetzt Wände, Bühne, Personen, Floor-Plan; Lampen bleiben')}><Icon name="import" size={15} />{t('topbar.importVenue', 'Venue importieren…')}</button>
              <div className="tb-dd-div" />
              <button className="tb-dd-item" onClick={run(p.onExportPlot)}><Icon name="schedule" size={15} />{t('topbar.printPlot', 'Lichtplan drucken (PDF, Titelblock + Legende)…')}</button>
              <button className="tb-dd-item" onClick={run(() => p.onExport('png'))}>{t('menu.exportPng', 'Export als PNG…')}</button>
              ...
              <div className="tb-dd-div" />
              <button className="tb-dd-item" onClick={run(p.onAbout)}><Icon name="info" size={15} />{t('menu.about', 'Über LightPlanner')}</button>
            </div>
          )}
        </div>
      </div>
      ...
      {/* ── center: mode switch ── */}
      <div className="tb-modeswitch" role="tablist" aria-label={t('menu.view', 'Ansicht')}>
        ...
        <button className={m === 'photo' ? 'on' : ''} onClick={() => p.onSetMode('photo')} title={t('topbar.renderModeTitle', 'Render: fotorealistische Vorschau der 3D-Szene (echte Scheinwerfer, Schatten, Lichtkegel, realistische Personen)')}><Icon name="photo" size={15} />{t('topbar.render', 'Render')}</button>
      </div>

      {/* ── right: display toggles, render settings, actions ── */}
      <div className="topbar-right">
        <button className={`tb-icon ${p.showHeatMap ? 'on' : ''}`} title={t('topbar.heatmapTitle', 'Heatmap (Beleuchtungsstärke einfärben)')} onClick={p.onToggleHeatMap}><Icon name="heatmap" /></button>

        <div className="tb-menuwrap">
          <button className={`tb-icon ${open === 'render' ? 'on' : ''}`} title={t('topbar.displaySettingsTitle', 'Anzeige & Render-Einstellungen')}
            ...
            <div className="tb-dropdown tb-render">
              {(p.viewMode === '3d' && p.photoMode) ? (
                <>
                  ...
                  <label className="tb-slider"><span>{t('topbar.exposure', 'Belichtung')}</span>
                  ...
                  <label className="tb-slider"><span>{t('topbar.ambience', 'Ambiente')}</span>
                  ...
                  <label className="tb-slider"><span>{t('topbar.haze', 'Dunst / Haze')}</span>
                  ...
                  <button className="tb-dd-item" onClick={p.onToggleBeams}><Icon name="beam" size={15} />{t('topbar.beams', 'Lichtkegel')}<span className={`tb-check ${p.showBeams ? 'on' : ''}`}><Icon name="check" size={13} /></span></button>
                  <div className="tb-dd-sec">{t('topbar.floorSection', 'Boden')}</div>
                  ...
                  <label className="tb-slider"><span>{t('topbar.floorColor', 'Bodenfarbe')}</span>
                ...
                <div className="tb-hint">{t('topbar.renderHintPre', 'Belichtung, Boden & Lichtkegel erscheinen im ')}<b>{t('topbar.render', 'Render')}</b>{t('topbar.renderHintPost', '-Modus.')}</div>
              )}
              {p.showHeatMap && (
                <>
                  <div className="tb-dd-sec">{t('topbar.heatmapSection', 'Heatmap')}</div>
                  <label className="tb-slider"><span>{t('topbar.scaleMax', 'Skala max')}</span>
                    <input type="number" min={10} max={100000} step={10} value={p.heatMapScale} onChange={(e) => p.onHeatMapScaleChange(+e.target.value)} />
                    <em>lx</em></label>
                  <label className="tb-slider"><span>{t('topbar.target', 'Zielwert')}</span>
                    <input type="number" min={0} max={100000} step={10} value={p.heatMapTarget} onChange={(e) => p.onHeatMapTargetChange(+e.target.value)} />
                    <em>lx</em></label>
                </>
              )}
              <div className="tb-dd-sec">{t('topbar.sunSection', 'Sonne / Tageslicht')}</div>
              <button className="tb-dd-item" onClick={() => p.onSunChange({ ...p.sun, enabled: !p.sun.enabled })} title={t('topbar.sunTitle', 'Echte Sonne: Tageslicht & Schatten aus Standort, Datum und Uhrzeit – fällt durch Fenster in den Raum.')}>
                <span className="tb-glyph">☀</span>{t('topbar.sunActive', 'Sonne aktiv')}<span className={`tb-check ${p.sun.enabled ? 'on' : ''}`}><Icon name="check" size={13} /></span>
              </button>
              {p.sun.enabled && (
                <>
                  <label className="tb-slider"><span>{t('topbar.date', 'Datum')}</span>
                    <input type="date" value={p.sun.date} onChange={(e) => p.onSunChange({ ...p.sun, date: e.target.value })} /></label>
                  <label className="tb-slider"><span>{t('topbar.time', 'Uhrzeit')}</span>
                    <input type="time" value={p.sun.time} onChange={(e) => p.onSunChange({ ...p.sun, time: e.target.value })} /></label>
                  <label className="tb-slider"><span>{t('topbar.latitude', 'Breite')}</span>
                  ...
                  <label className="tb-slider"><span>{t('topbar.longitude', 'Länge')}</span>
                  ...
                  <label className="tb-slider"><span>{t('topbar.north', 'Norden ↻')}</span>
                  ...
                  <label className="tb-slider"><span>{t('topbar.intensity', 'Intensität')}</span>
                  ...
                  <div className="tb-hint">{p.sunInfo ? `${t('topbar.sunAltitude', 'Sonnenstand')}: ${p.sunInfo.altitudeDeg.toFixed(0)}° ${t('topbar.sunAboveHorizon', 'über dem Horizont')} · ${t('topbar.sunAzimuth', 'Azimut')} ${p.sunInfo.azimuthDeg.toFixed(0)}° (0 = N). ${t('topbar.sunThroughWindows', 'Fällt durch Fenster in den Raum.')}` : t('topbar.sunBelowHorizon', 'Sonne steht unter dem Horizont – kein direktes Tageslicht.')}</div>
                </>
              )}
              <div className="tb-dd-div" />
              <button className="tb-dd-item" onClick={p.onToggleSnap}><Icon name="snap" size={15} />{t('topbar.snap', 'Einrasten')}<span className={`tb-check ${p.snapStep > 0 ? 'on' : ''}`}><Icon name="check" size={13} /></span></button>
              <button className="tb-dd-item" onClick={p.onToggleFocusNotes} title={t('topbar.focusNotesTitle', 'Fokus-Notizen je Scheinwerfer im 2D-Plan einblenden')}><Icon name="tag" size={15} />{t('topbar.focusNotes', 'Fokus-Notizen (Plan)')}<span className={`tb-check ${p.showFocusNotes ? 'on' : ''}`}><Icon name="check" size={13} /></span></button>
            </div>
          )}
        </div>

        <button className="tb-icon" title={t('topbar.importFloorPlanTitle', 'Grundriss importieren (JPG/PNG/PDF)')} onClick={() => fileRef.current?.click()}><Icon name="import" /></button>
