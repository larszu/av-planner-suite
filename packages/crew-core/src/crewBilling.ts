...
// ───────────────────────────────────────────────────────────────────────────
/**
 * KEINE CSV-Abhaengigkeit (ADR-006, Schritt 2).
 *
 * `lib/csv.ts` des Cable-Planers hat 19 Nutzer quer durch den Planer — es ist
 * generische Infrastruktur wie `mergeDefined` beim Lager und gehoert keiner
 * Domaene. Dieses Paket erzeugt deshalb eine TABELLE; wer daraus eine Datei
 * macht, entscheidet der Aufrufer. Eine zweite Fassung von `toCsv` hier waere
 * genau die Doppelung, gegen die ADR-001 geschrieben ist.
 */
export type CrewZelle = string | number | null | undefined

/** Kopfzeile plus Datenzeilen — ein Dokument, bevor es CSV wird. */
export interface CrewTabelle {
  headers: string[]
  rows: CrewZelle[][]
}

import { expenseTotals, formatHours, labourCosts, labourFindings } from './labourCost'
import { EXPENSE_KIND_LABEL, type CrewExpense, type CrewPlan } from './labour'
 ...
 */
export const crewBillingTable = (b: CrewBilling): CrewTabelle => {
  const rows: CrewZelle[][] = b.rows.map((r) => [
    'Arbeit',
    r.personName,
    r.company ?? '',
    r.activity,
