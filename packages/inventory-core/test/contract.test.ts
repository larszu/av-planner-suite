...
//
// Das Format lebt seit dem Monorepo-Zusammenzug EINMAL in diesem Paket
// (@avplan/inventory-core) und wird von allen Apps (cable / multicam / light)
// konsumiert. Diese Datei friert den Wire-Contract ein: Format-Marker,
// Versionsnummer, Envelope-Shape und die Feld-Namen jeder Entitaet — damit
// bereits exportierte Lager-Dateien app-uebergreifend importierbar bleiben.
//
// !!! Wenn dieser Contract bewusst geaendert wird:
//   1. INVENTORY_FORMAT_VERSION erhoehen (Abwaertskompatibilitaet beachten),
//   2. die eingefrorenen Key-Listen unten anpassen.
// ───────────────────────────────────────────────────────────────────────────
...
describe('avplan-inventory Wire-Contract (Drift-Guard)', () => {
  ...
  it('Envelope-Shape ist eingefroren', () => {
    const file = JSON.parse(serializeInventory(snapshot, { exportedAt: 't', app: 'inventory-core' }))
  ...
  it('resolveInventoryCode: Unit vor Node vor Item, case-insensitiv, leer -> null', () => {
    ...
    expect(resolveInventoryCode('unbekannt', src)).toBeNull()
  })
})
