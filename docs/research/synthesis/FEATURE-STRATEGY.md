...
| 8 | Netz-/IP-Plan | 19 | **gebaut, bis auf die Vergabe** | `lib/subnet.ts` + IPAM-Übersicht + NetBox-Import, dazu seit `cable#701` der **abgeleitete Adressplan** (`lib/addressPlan.ts`): welches Gerät braucht eine Adresse (aus seinen Ports abgeleitet, mit Beleg, welcher Port es sagt), und fünf nachrechenbare Befunde — keine Adresse, doppelte Adresse, fehlende Maske, Gateway außerhalb des Subnetzes, Netz-/Broadcast-Adresse (nicht auf /31 und /32, RFC 3021). Die alte Ansicht zeigte nur, was **ausgefüllt** ist; ihre Zeilenauswahl konnte per Konstruktion nicht zeigen, was fehlt. **Offen bleibt bewusst das Vergeben:** woher Subnetze kommen, ist E-5, und ein Werkzeug, das währenddessen selbst welche vergibt, entscheidet die Frage stillschweigend |
| 9 | Delivery-/Streaming-Kette | 18 | **gebaut bis zur Ausspielung** (nachgemessen 2026-09-06) | Die Contribution-Hälfte stand schon: NDI, NDI-HX, Dante, AES67 und ST2110-20/30/40 sind Mitglieder der `SignalStandard`-Union, hängen an Ports und Kabeln, tragen Bandbreite und Impedanz, und der Plan rechnet damit (Netz-Budget mit Link-Tier-Empfehlung, ST-2110-ohne-PTP, Impedanz-Bruch, passive Längengrenze). **Die in Runde 9 als fehlend benannte Delivery-Hälfte ist inzwischen da:** `SRT`, `RTMP` und `HLS` sind eigene `SignalStandard`-Werte mit je einer belegten Bandbreite (SRT 12 Mbps mit Retransmit-Reserve, RTMP 6 Mbps nach der Richtangabe der Plattformen, HLS 10 Mbps für die Ausspiel-Leiter), und der Katalog führt den Stream-Uplink als Gerät. **Offen bleibt genau ein Stück:** das CDN als Topologie-Objekt — wohin ausgespielt wird und mit welchem Rückfall — kommt im Quelltext nicht vor. Die Kette endet am Uplink |
