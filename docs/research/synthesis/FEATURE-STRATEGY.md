...
| 3 | Stückliste / Kommissionier-Liste | 24 | **fertig** | `lib/planBom.ts` (ADR-002 Inkrement 4). Der Ausweg aus einer geratenen Zeile hat seit `cable#767` **beide** Hälften: „Bestätigen" schreibt die Katalog-Identität auf die Lager-Position, „Typ zuweisen…" auf alle Plan-Geräte der Zeile. Die zweite deckt den Fall ab, den ADR-002 unterschätzt hatte — bei einer Lager-Position MIT Identität und einem Plan-Gerät ohne greift der Namensvergleich gar nicht, die Zeile steht als `unmatched` da, und die Kamera fehlt auf der Liste, während sie im Regal steht |
...
| 20 | Schichtbare Adressbereiche: stehender Plan + Haus-Ebene, mit Ueberlappungs-, Klammer- und VLAN-Pruefung und einem Umzugs-VORSCHLAG (P1) | `cable#750` |
| 8 · 6 | Ablauf einlesen (tolerant, mit Spalten-Zuordnung und Vorschau) und ueber Objekt-Ids mit dem Plan verknuepfen (P1) | `suite#142` |
...
| 4 | Ablauf-Interoperabilitaet mit der Tabelle, in der er lebt (P1) | `suite#142` (CSV/TSV) + `suite#167` (XLSX). Die Mappe ist das, was der Kunde schickt; wer sie erst in Excel oeffnen und als CSV speichern muss, hat den Bedarf verschoben statt erfuellt. Die tragende Stelle ist die Uhrzeit: Excel legt „14:20" als 0.5972… ab, und ein roher Leser bekommt die Zahl, `parseClock` gibt `null`, der Ablauf steht als „ohne Zeit" da — jede Zeile eingelesen, nichts fehlt, und trotzdem falsch. Gelesen wird deshalb mit dem Zahlenformat der Zelle. Ausgabe in beiden Formaten aus **einer** Zeilen-Tabelle (`rundownViewRows`), damit die zwei Blaetter nicht auseinanderlaufen |
...
| 97 | Aus einem abfotografierten Beleg wird eine gefuellte Auslagenzeile — an DER ZEILE und am Job (P3) | `cable#757` (`lib/receiptRead.ts`, `main/services/receiptStore.ts`, `lib/receiptChain.ts`) — die Massnahme woertlich: „attach the receipt to the expense line, not the parent, and hang the line on the project“. Kein OCR (offline-first; die einzige brauchbare Texterkennung liegt bei einem Cloud-Dienst): gelesen wird eingefuegter Text, der EXIF-Aufnahmezeitpunkt und der Dateiname, jedes Feld mit Herkunft. Zwei Summenkandidaten ergeben KEINEN Betrag, sondern beide zur Auswahl — „Gegeben“ und „Rueckgeld“ sind nie die Rechnungssumme |
| 108 | Eine Seite, die sagt, was fällig, überfällig, in Konflikt oder über Budget ist — statt eines täglichen Rundgangs (P3) | `cable#758` (`lib/actionItems.ts`) — der Bedarf sagt ausdrücklich „a notification-and-derivation product, NOT an authoring product": die Seite sammelt aus sechs vorhandenen Stellen ein und rechnet selbst nichts. Fälligkeit steht nur da, wo die Quelle ein Datum HAT; was keins hat, ist `undated` und nicht „heute fällig" |
| 100 | Aufnahmenamen aus dem Plan statt Deck für Deck, mit sichtbarem Fehlschlag (P3) | `cable#759` (`lib/recordNaming.ts`) — „automated filename building fails silently while the button label still renders correctly". Gebildet wird der Name, den das Gerät TATSÄCHLICH bekäme, samt Prüfung: doppelter Name, abgelehntes Zeichen, zu lang. Eine Take-Nummer je PROJEKT, nicht je Deck. **Nicht gebaut:** Push an die Decks und Zustands-Rückmeldung — beides braucht eine Verbindung zu jedem Gerät, und eine Anzeige, die nur den Plan zeigt, wäre der Defekt des Belegs |
| 109 | Lesen viele, schreiben einer — nichts, was hinausgeht, wird zur veralteten Abspaltung (P3) | `cable#759` (`mobileShareServer`) — die Lese-Hälfte gab es längst, die drei Schreibwege waren immer offen. Jetzt `writeMode` mit Vorgabe `read-only`, geprüft an EINER Stelle vor dem Lesen des Bodys, 403 statt 401, und das Handy blendet seine Schreib-Bedienung aus statt sie ins Leere laufen zu lassen |
| 98 | Ein Custody-Modell, das der Halle folgt: rückwirkend beginnen, mitten im Lauf verlängern (P3) | `cable#759` (`lib/custodyPeriod.ts`) — „starting in past, for items picked up in hurry" und „edit bookings end date while it's going on". Rückwirkend ja ohne Frist, vorauswirkend nein (das wäre eine Reservierung); verschieben in beide Richtungen; der alte Termin bleibt in `extensions` stehen. Die Vokabel „Load out/Load in" NICHT übernommen — die deutsche Oberfläche sagt „Ausgabe"/„Rückgabe", und das ist bereits das Hallenwort |
