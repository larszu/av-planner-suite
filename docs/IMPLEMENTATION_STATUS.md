...
| Ein universelles Geraet statt drei Listen | `PARTIAL` | B-80 / ADR-011, Eigentuemer-Entscheidung 2026-09-19. **Stufe 1 von 4 gebaut:** `geraete` ist die Wahrheit im Seed, `cameras`/`fixtures`/`devices` sind Sichten darauf (`packages/ui/src/geraet.ts`), die Kategorie ordnet ein Geraet mehreren Plaenen zugleich zu, und der Signalplan bekommt seither ALLE Geraete. Zusammengelegt wird nur ueber die erklaerte Entsprechung (`represents`) — geraten wird nichts. **Offen:** das Shell-Projekt fuehrt weiter drei Listen (Stufe 2), die Planer lesen weiter ihre Sicht (Stufe 3), das Geruest `altIds` und die Rueckweg-Sonderregel fallen erst danach (Stufe 4). Guards: `packages/ui/test/geraet.test.ts`, `apps/shell/test/kategorieAussage.test.ts` |
| Kamera aus dem Signalplan im Kameraplan | `IMPLEMENTED` | B-79, Nutzer-Auftrag 2026-09-19. Der Cable-Planer erklaert `SeedDevice.kategorie` aus seinem KATALOG (nie aus dem Namen, ADR-002; seit ADR-011 dieselbe Angabe, die auch die Plan-Zuordnung traegt) und schickt endlich das Modell mit; die Shell haelt beides am Knoten fest und bietet die Uebernahme als Streifen an (`data/kameraUebergabe.ts`, `shell/KameraUebergabeBar.tsx`). Uebernehmen legt die Kamera an UND setzt `represents` — sonst zaehlte der Bedarf Knoten und Kamera doppelt. Eine Position wird NICHT geraten: `Camera.x/y` sind optional, und der Rueckweg setzt keine Null mehr. Guards: `test/kameraUebergabe.test.ts`, `test/gewerksAussage.test.ts`, zwei Faelle in Cables `shellSeed.test.ts`. Nicht `COMPLETE`, weil der Kameraplan ein Modell auslaesst, das er nicht eindeutig trifft, und diese Meldung bisher nur in der Konsole steht |
