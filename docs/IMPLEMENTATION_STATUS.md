...
| Kamera aus dem Signalplan im Kameraplan | `IMPLEMENTED` | B-79, Nutzer-Auftrag 2026-09-19. Der Cable-Planer erklaert `SeedDevice.gewerk` aus seinem KATALOG (nie aus dem Namen, ADR-002) und schickt endlich das Modell mit; die Shell haelt beides am Knoten fest und bietet die Uebernahme als Streifen an (`data/kameraUebergabe.ts`, `shell/KameraUebergabeBar.tsx`). Uebernehmen legt die Kamera an UND setzt `represents` — sonst zaehlte der Bedarf Knoten und Kamera doppelt. Eine Position wird NICHT geraten: `Camera.x/y` sind optional, und der Rueckweg setzt keine Null mehr. Guards: `test/kameraUebergabe.test.ts`, `test/gewerksAussage.test.ts`, zwei Faelle in Cables `shellSeed.test.ts`. Nicht `COMPLETE`, weil der Kameraplan ein Modell auslaesst, das er nicht eindeutig trifft, und diese Meldung bisher nur in der Konsole steht |
