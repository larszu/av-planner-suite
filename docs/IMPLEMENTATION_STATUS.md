...
| Kette | Status | Beleg |
| --- | --- | --- |
| Plan → Tally-Pi | `PARTIAL` | Datenvertrag `COMPLETE` und getestet. **Aus der Suite laeuft der Transport** seit `suite#99`: `apps/shell/electron/tallyPush.cjs` schickt die Karte per `POST /tally-config` an den Pi, mit Abgleich davor — aus dem Hauptprozess und nicht per `fetch` aus dem Renderer, weil der Pi-Server weder CORS-Kopfzeilen noch einen OPTIONS-Zweig hat. **Aus dem `cable-planner` selbst bleibt es ein Download** (`ExportDialog.tsx`), den jemand nach `/opt/pi-guide/tally.json` kopiert — ob der Planer direkt liefern soll oder die Datei der Weg bleibt, ist die offene Eigentuemer-Frage E-7 (B-6) |
