...
| Ein universelles Geraet statt drei Listen | `PARTIAL` | B-80 / ADR-011, Eigentuemer-Entscheidung 2026-09-19. **Stufen 1 bis 3 von 4 gebaut:** `geraete` ist die Wahrheit im Seed, `cameras`/`fixtures`/`devices` sind Sichten darauf (`packages/ui/src/geraet.ts`), die Kategorie ordnet ein Geraet mehreren Plaenen zugleich zu, und der Signalplan bekommt seither ALLE Geraete. Zusammengelegt wird nur ueber die erklaerte Entsprechung (`represents`) — geraten wird nichts. Seit Stufe 2 fuehrt das SHELL-PROJEKT eine Liste: `represents`, `altIds` und die Rueckweg-Sonderregel sind WEG, die Kamera-Uebergabe ist geloescht (sie beantwortete eine Frage, die nur im Drei-Listen-Modell entstand; ihr Backlog-Eintrag erklaert, warum es sie gab), und das Eigentum steht je FELDGRUPPE statt je Liste. Alte Projektdateien migriert `projectFile.ts` ueber die damals erklaerte Zuordnung. Seit Stufe 3 lesen alle drei Planer `seed.geraete` und melden `patch.geraete`; die Sichten werden GERECHNET statt uebertragen. **Offen:** die Sichten fallen aus dem Seed (Stufe 4). Guards: `packages/ui/test/geraet.test.ts`, `apps/shell/test/kategorieAussage.test.ts` |
