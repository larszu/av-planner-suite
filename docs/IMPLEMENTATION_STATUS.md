...
**Stand: 2026-09-08.** Erhoben durch **Ausführung**, nicht durch Lesen von Plänen:
...
| Green-GO Round-Trip | `COMPLETE` | Preset überlebt den Export inkl. `ButtonFunctions` (`cable#653`); das **herstellerneutrale** Format kam mit `cable#684` (B-8, in die Suite vendoriert mit `suite#86`): `avplan-intercom` v1 mit Sprechstellen, Konferenzen und `IntercomMembership {channelId, talk, listen}`. Talk und Listen getrennt, weil genau diese Trennung der Punkt eines Intercom-Systems ist. Beide Knöpfe stehen nebeneinander im `GreenGoExportDialog` — `.gg5` ist der Weg IN die Anlage, das neutrale Format der Weg aus dem Haus, den auch jemand liest, der Riedel oder Clear-Com aufbaut. Ein Format, das nur im Code existiert, ist keines |
...
| Ausspielweg (SRT/RTMP/HLS) | `COMPLETE` | `cable#683`; die drei sind vollwertige Signal-Standards mit Bandbreite und zaehlen als **Last** im Netz-Budget, dazu ein Katalog-Kabel. Die zweite Haelfte — das ZIEL jenseits des Hauses — kam mit `cable#703`/`#707`/`#710` (B-10): `types/delivery.ts` fuehrt `DeliveryDestination` mit Transport, `EncodingProfile`, Modus und Plattform, am Projekt haengt sie als `project.deliveryDestinations`, und `lib/deliveryPath.ts` haengt sie ueber `encoderEquipmentId` an den Kabelgraph — ein Ausspielziel ist damit ein Endpunkt IM Plan und kein zweiter Graph daneben. Der Stream-Key bleibt ausdruecklich draussen (Credential-Store) |
...
| Versionen & Vergleich (light) | `COMPLETE` | Sichern, Vergleichen und Wiederherstellen sind echt; die Identität ist seit `light#57` stabil, Grundriss und Fremd-Domänen überleben seit `light#58`. Der Vergleich sah 6 von 14 Kategorien (B-21) — seit `light#91` sagt er wenigstens, was er NICHT prüft, seit `light#95` prüft er alle vierzehn. `ALLE_KATEGORIEN` ist die eine Liste, aus der Summe, Ansicht und Zusammenfassung ziehen; `unnamedDifferences` rechnet den Rest aus, sodass ein neues Projekt-Feld ohne Vergleich sich selbst meldet |
| Tab-Leiste | `COMPLETE` | eigene Views sind echt (`OverviewSurface`, `BoardCanvas`, `previews.tsx`). Der Tab-**Wechsel** war ein No-Op — `activeTab` kam an vier Stellen vor, alle vier nur Anzeige. Mit `suite#100` sind die wirkungslosen Tabs **entfernt statt ausgebaut** (E-9): es waren dreizehn von dreizehn, die drei Planer bringen ihre Ansichtsleiste selbst mit, und fuer die uebrigen haette Ausbauen geheissen, Ansichten zu erfinden (B-16) |
| Cross-Link Planer → Shell | `COMPLETE` | `suite#199` (B-18). Der `post*`-Helfer, der fehlte, heisst `postNavigateToShell` (`packages/ui/src/embed.ts`), ist aus `@avplan/ui` exportiert, und die Shell nimmt ihn in `App.tsx` entgegen. `test/sprungZiel.test.ts` deckt ihn ab. **Nachgemessen 2026-09-09** — die Zeile stand bis dahin auf `MISSING` und schickte damit jemanden los, etwas zu bauen, das schon stand |
...
| Sprachumschaltung Shell → `multicam-planner` | `COMPLETE` | `multicam#116` (B-25, E-20). Upstream gibt es die i18n jetzt (`src/i18n/`), Englisch ist Quellsprache und wird gemessen: `npm run lang:check` meldet dort 0 ungewickelte deutsche Zeichenketten. **Nachgemessen 2026-09-09** — die Zeile behauptete bis dahin, upstream gebe es gar keine i18n |
...
| Lexware — Beleg anlegen | `COMPLETE` | `suite#187` (B-19, E-12). Der Widerspruch ist aufgeloest, weil der Weg umgezogen ist: Lexware ist eine eigene **Shell-Domaene** (`packages/lexware-core`, `suiteHost:lexware:*` im Hauptprozess), der Planer-Handler entfaellt. Der Schluessel liegt im Schluesselbund der Desktop-App; im Browser gibt es ihn nicht, und die Oberflaeche sagt das. `test/lexwareShellDomaene.test.ts`. **Nachgemessen 2026-09-09** |
...
| Ein universelles Geraet statt drei Listen | `DONE` | B-80 / ADR-011, Eigentuemer-Entscheidung 2026-09-19. **Alle vier Stufen gebaut.** `geraete` ist die einzige Geraeteliste — im Seed, im Shell-Projekt und im Rueckweg —, und ein Plan ist ein FILTER darauf (`imKameraplan`, `imLichtplan`, `imSignalplan` in `packages/ui/src/geraet.ts`). Die Kategorie ordnet ein Geraet mehreren Plaenen zugleich zu; der Signalplan bekommt ALLE Geraete. `represents`, `altIds`, die Rueckweg-Sonderregel, die Kamera-Uebergabe und die Typen `SeedCamera`/`SeedFixture`/`SeedDevice` sind WEG; das Eigentum steht je FELDGRUPPE statt je Liste. Alte Projektdateien migriert `projectFile.ts` ueber die damals erklaerte Zuordnung. Zwei Befunde aus Stufe 4: Lager und Gebaeude durften Geraete melden (ADR-006 in der Gegenrichtung), und die leere Feldgruppe `kamera: {}` zog fremde Geraete in den Kameraplan — beides gefixt und mit Test festgehalten. Guards: `packages/ui/test/geraet.test.ts`, `packages/ui/test/seed.test.ts`, `apps/shell/test/kategorieAussage.test.ts` |
| Eine Basis fuer alle Geraetetypen | `DONE` | B-81 / ADR-012, Eigentuemer-Entscheidung 2026-09-19 (alles teilt dieselbe Basis). `SeedGeraet.typId` traegt die Katalog-Identitaet durch den Seed; `Geraetetyp.refs` traegt die Rueckrichtung, sodass jeder Planer seinen EIGENEN Eintrag zu einer geteilten Id findet, ohne Namen zu vergleichen. Die drei Aufloeser (`katalogTemplate`, `katalogKamera`, `katalogFixture`) fragen die Id VOR dem Namen; `deriveBedarf` schluesselt darueber, und das Lager trifft damit die Position statt ueber `model.toLowerCase()`. Die 835 Objektive stehen als Kategorie `Lenses` im Katalog (1751 Typen gesamt). Das Lager bekommt den Katalog ueber einen ANSCHLUSS (`lib/typKatalog.ts`), weil es auch als eigene App laeuft, wo das Paket nicht danebenliegt. Nebenbefund gefixt: die Kategorie `Sync/Reference` gab es zweimal (eine Katalogdatei war beim Sprachwechsel #822 uebersehen). Der facility-planner bekommt KEINE Liste - seine Aufzaehlungen sind Normbegriffe, kein Katalog (in ADR-012 begruendet). Guards: `device-catalog/test/rueckref.test.ts`, `device-catalog/test/kategorien.test.ts`, `ui/test/bedarfTypId.test.ts`, `shell/test/typBasis.test.ts` |
| Fachdaten ueberleben den Weg von Planer zu Planer | `DONE` | B-82 / ADR-013, Eigentuemer-Meldung 2026-09-19: die Fachdaten muessen in ALLEN Planern bleiben, damit ein Projekt von a nach b nach c und wieder nach a laufen kann. `SeedGeraet.fachdaten` traegt ein Fach je Gewerk: jeder schreibt seines vollstaendig, niemand liest ein fremdes, niemand verliert eines. Gemessen war der Verlust echt — eine ausgerichtete Kamera kam mit DREI Feldern zurueck (Objektiv, Brennweite, Bildwinkel); Schwenk, Neigung, Hoehe, Blende, Fokus und Farbe fielen heraus, im Signalplan die ANSCHLUESSE. In einer Sitzung fiel es nicht auf, weil jeder Planer seinen eigenen Stand darueberlegt; ueber die DATEI war es weg. Regel dagegen: was das Protokoll fuehrt, gehoert nicht ins Fach (je Planer eine Ausschlussliste `GETEILT`, je ein Test). Guards: `ui/test/fachdaten.test.ts`, `shell/test/rundlaufDreiPlaner.test.ts` (ueber Shell UND Datei), je Planer ein `fachdatenRundlauf.test.ts`. Gegenprobe gefahren: ohne Fach steht die Kamera wieder auf -90 Grad. |
| Selbst angelegte Geraete in allen Planern | `DONE` | B-83 / ADR-014, Eigentuemer-Meldung 2026-09-19. Ein selbst angelegtes Modell faehrt als `eigenesModell`/`eigenesObjektiv` im Fach mit - vorher meldete der Rueckweg es OHNE `model` (`CAMERAS.find` kennt eigene Modelle nicht), und die Kamera fiel beim naechsten Seed heraus. Ein von Hand im Signalplan angelegtes Geraet wurde von den Fachplanern verworfen, mit einer Zeile in der Konsole; jetzt fuehrt `ausgelassen` das GERAET, beide Planer halten die Liste im Zustand und zeigen sie. Geraten wird weiter nichts: ohne Sensor kein Bildwinkel, ohne Photometrie keine Rechnung - dieselbe Form wie `portsUnknown` im Cable-Planer. Guards: `multicam/__tests__/eigenesGeraet.test.ts`, `shell/test/eigenesGeraetDurchgereicht.test.ts`. |
| Crew & Geld als eigenes Werkzeug (ADR-006) | `PARTIAL` | B-84. Schritte 1 und 2 von 3 gebaut am 2026-09-19. Der VERTRAG steht in ADR-006 (fuenf Fragen, je mit Namen und Aufrufer, am Import-Querschnitt gemessen); das PAKET `@avplan/crew-core` traegt `labour.ts`, `labourCost.ts`, `crewBilling.ts`, `crewCalendar.ts`, `receipt.ts`, und der Cable-Planer bezieht sie von dort (die alten Pfade sind Durchreicher). Der Schnitt war sauber, weil `types/labour.ts` KEINEN Import hatte - diese Domaene hing nie am Kabelgraph. NICHT mitgegangen: `csv.ts` (19 Nutzer, generische Infrastruktur), `costComparison.ts` (liest `project.equipment`) und `crewNetworkSheet.ts` (der Name sagt Crew, der Inhalt ist der Kabelgraph). Schritt 3 (eigenes Repo) offen: erst, wenn ein zweiter Bediener die Domaene SCHREIBT statt nur liest. Guards: `crew-core/test/grenze.test.ts`, `crew-core/scripts/plan-grenze-check.mjs`. |
| Kamera aus dem Signalplan im Kameraplan | `COMPLETE` | B-79, Nutzer-Auftrag 2026-09-19. Der Cable-Planer erklaert `SeedDevice.kategorie` aus seinem KATALOG (nie aus dem Namen, ADR-002; seit ADR-011 dieselbe Angabe, die auch die Plan-Zuordnung traegt) und schickt endlich das Modell mit; die Shell haelt beides am Knoten fest und bietet die Uebernahme als Streifen an (`data/kameraUebergabe.ts`, `shell/KameraUebergabeBar.tsx`). Uebernehmen legt die Kamera an UND setzt `represents` — sonst zaehlte der Bedarf Knoten und Kamera doppelt. Eine Position wird NICHT geraten: `Camera.x/y` sind optional, und der Rueckweg setzt keine Null mehr. Guards: `test/kameraUebergabe.test.ts`, `test/gewerksAussage.test.ts`, zwei Faelle in Cables `shellSeed.test.ts`. **Am selben Tag ueberholt:** mit B-80 (ADR-011 Stufe 2) fuehrt die Shell EINE Geraeteliste, und ein Geraet der Kategorie „Cameras" steht damit ohne Zutun in beiden Plaenen. Die Uebergabe samt Streifen ist geloescht; der Backlog-Eintrag erklaert, warum es sie gab |
...
| Nachschlagen mitten in der Show | `IMPLEMENTED` | Bedarf 76, `cable#720` — „what's the IP of X, which switch port, which VLAN". Der Bruch war nicht, dass die Suche fehlte: `CanvasSearch` traf fuenf PROSA-Felder, und wer „10.20.0.14" eintippte, bekam nichts, obwohl der Plan die Adresse kennt. Beide Haelften der Antwort lagen fertig da und wurden nicht erreicht — `deviceInterfaces` (Bedarf 19) und `buildSwitchPortMaps` (Bedarf 24). `lookupInPlan` ist die Engstelle; jeder Treffer nennt das FELD, auf das er traf, weil ein Fund in einer Notiz mit der ALTEN Adresse sonst wie der Adress-Traeger aussieht. Was dranhaengt, kommt aus dem Kabelgraph. **„Is it flowing" wird nicht beantwortet** — ein offline-Planer weiss das nicht, und eine geratene Antwort saehe wie eine Messung aus |
| Die Antwort der Haus-IT | `IMPLEMENTED` | Bedarf 85 (zweite Haelfte), `cable#720` — „the outcome (what was opened, what was refused, what the workaround was) is never recorded". Bedarf 23 hatte die Frage gebaut. Kein Ja/Nein: der haeufigste Ausgang ist „mit Auflage", und beide Kreuze waeren beim naechsten Mal falsch. Drei Faelle mit eigenem Namen — `elsewhere` (die Antwort kommt aus einem anderen Haus; der Ort steht IN der Antwort, beim Schreiben eingefroren wie `CheckoutLine.ownership`, und wird streng verglichen), `stale` (Antwort auf einen Punkt, den der Plan nicht mehr stellt — Auskunft ueber das Haus, kein Muell) und `pending` (gefragt, nichts gehoert). Geraten wird nichts, und nichts wird an die Anforderung zurueckgeschrieben: zwei Herkuenfte, zwei Spalten |
| Etiketten, die das Lager ueberstehen | `IMPLEMENTED` | Bedarf 70 — **nicht neu gebaut, sondern 2026-09-06 als gebaut nachgewiesen.** Die drei Forderungen sind erfuellt: der Code steht als TEXT unter dem Symbol (auch beim QR, nicht nur beim Barcode, `labelSheets.ts`), es gibt acht Geometrien statt der verlangten drei (vier Avery/Zweckform-Boegen aus den Datenblaettern, vier Endlos-Rollen), und das Abtippen ist der HAUPTWEG und nicht die Strafe: beide Scan-Flaechen (Ausgabeschein, Inventur) sind Textfelder, die Kamera ist die Zugabe |
| Die Zeit im Netz-Modell | `IMPLEMENTED` | Bedarf 73, `cable#721` — „PTP domain, grandmaster and boundary-clock topology live nowhere", und der Schaden mit Zahlen: „ST 2059-2 defaults to domain 127 while AES67 commonly uses domain 0". Drei Felder an der SCHNITTSTELLE (ein redundanter 2110-Aufbau faehrt rot und blau bewusst getrennt); `ptpPlan.ts` gruppiert nach Domaene, leitet die Essenz-Familie aus dem KABELGRAPH ab und liefert fuenf benannte Befunde. `unspecified` ist nirgends ein Befund, und kein Feld wird vorbelegt — eine geratene Vorgabe waere genau der Widerspruch, den die Pruefung sucht. Zwei echte Lade-Pfad-Fehler mitrepariert: `interfaceIsEmpty` kannte die Felder nicht (eine Schnittstelle mit NUR einer Domaene galt als leer und flog raus), `normaliseNetworkInterface` uebernahm sie nicht |
| Das Netz-Merkblatt fuer die Crew | `IMPLEMENTED` | Bedarf 77, `cable#721` — „today it is a verbal briefing plus a WhatsApp message". Der Unterschied zur Nachricht liegt NICHT im Inhalt, sondern im Stand: das Blatt ist ein Dokument nach ADR-004 (`crew-netz`), also beantwortet `sheetLookup` (Bedarf 27) die Frage „gilt das noch?". Fuenf Abschnitte aus Quellen, die es ohnehin gibt. Die Regel, die den Inhalt bestimmt: was der Plan nicht weiss, steht als FRAGE drauf und nicht als Luecke — eine leere Zeile liest sich wie „gibt es nicht", und dann steckt jemand ein Kabel. Ohne Geraete-Adressen (dafuer gibt es das Rack-Tuer-Blatt) und nur mit den GRENZEN aus den Haus-Antworten |
| Ein Spektrum-Plan | `IMPLEMENTED` | Bedarf 95, `cable#721` — „one wireless plan, not separate RF and comms features". **Der Befund lag im eigenen Haus:** der Planer rechnete Intermodulation ZWEIMAL und beide Male auf der halben Menge — `deriveRig` ueber die Rig-Kanaele, der RF-Reiter ueber die Kabel-Strecken, mit einer von Hand nachgebauten IM3-Schleife und einer eigenen Konstante. Eine Mikrofonstrecke auf 606,400 MHz und ein Kameralink auf 606,500 MHz begegneten sich in KEINER der beiden. Intermodulation auf der halben Senderliste ist schlimmer als gar keine: keine Rechnung sagt nichts, eine halbe sagt „frei". `spectrumPlan.ts` ist die Engstelle; jeder Sender nennt Herkunft und TRAEGER (der Intercom-Teilnehmer haengt ueber `equipmentId` am Geraet — die Antwort auf „who carries beltpack 7"), und der Umfang der Rechnung steht ueber ihrem Ergebnis |
| Die Crew-Seite eines Jobs | `IMPLEMENTED` | Bedarfe 40, 41, 42, 83 und 39, `cable#756`. Fuenf P2-Bedarfe, ein Modell — sie betreffen dieselbe Zahl: was die Arbeit gekostet hat und woher sie kommt. **Der Beleg zu 40 ist eine Rechnung, die schiefging:** „10 h zu 50 und 2 h zu 55" wurde als „12 x 50" gerechnet (`kimai#5913`, sechs Melder, 2018-2026). `lib/labourCost.ts` rechnet deshalb MINUTENWEISE und ist die einzige Stelle, die aus Schichten Geld macht. Drei Regeln stehen als benannte Konstanten statt im Code: Baender stapeln NICHT (hoechster Zuschlag je Minute), Ueberstunden- und Bandzuschlag ADDIEREN sich (sie messen Verschiedenes), die Pauschale faellt einmal je Person und Kalendertag. **Ein Feld fehlt mit Absicht:** `breakMinutes` — eine Pause als Zahl zwingt die Rechnung zu entscheiden, WELCHE Minuten unbezahlt sind, und das aendert das Geld; eine Pause ist eine Luecke zwischen zwei Eintraegen. Bedarf 42 ist Einfuegen und Zuordnen, KEIN Abfangen (so verlangt): steht kein Zeitstempel im eingefuegten Text, bekommt die Zusage keinen — die strittige Frage ist „ab wann war die Mehrarbeit freigegeben". Bedarf 39 liefert einen ABONNIERBAREN Feed (`/crew.ics`, token-gated) statt eines Downloads; „vorgemerkt" und „reserviert" sind in RFC 5545 beide TENTATIVE, der Unterschied steht im Titel. Die drei Zustandsnamen sind UNBELEGT uebernommen — die Massnahme verlangt eine Pruefung, die hinter dem Egress-Filter liegt |
| Der Beleg haengt an der Auslagenzeile | `IMPLEMENTED` | Bedarf 97, `cable#757` — „Images attach to the parent record, UNLINKED TO THE LINE. Trips produce no accounting consequence at all“ (`frappe/hrms#4541`). Beide Haelften der Massnahme sind gebaut: die Datei haengt an `CrewExpense.receipt` (nicht am Projekt) und `costLineId` haengt die Zeile an eine Kostenposition. **Die Datei liegt in `Belege/` NEBEN dem Projekt**, unter dem SHA-256 ihres Inhalts — ein Foto von zwei Megabyte in jeder `.avplan` verteuerte jede Speicherung und jeden Versand; der Preis dieser Entscheidung ist benannt (`missing`, wenn der Ordner nicht mitgereist ist). **Kein OCR und kein Cloud-Dienst:** gelesen wird, was ohne Bilderkennung maschinenlesbar dasteht — eingefuegter Text, EXIF `DateTimeOriginal` (von Hand geparst, mit Grenzenpruefung), Dateiname, Dateidatum; jedes Feld traegt seine Herkunft, und ein Datum aus dem Dateidatum wird nie unkommentiert uebernommen. Die wichtigste Zeile ist die Ausschlussliste: wer die letzte Zahl vom Bon nimmt, traegt das Wechselgeld als Rechnungsbetrag ein. `CostLine.actual` wird NIE gerechnet — die belegte Summe steht daneben, und wer sie uebernimmt, traegt `from-invoice` als Herkunft. Nebenbei: `atomicWriteFile` nimmt jetzt auch Binaerdaten, weil die Regel „Userdaten immer atomic“ keine Ausnahme fuer Bilder kennt |
| Eine Seite, die sagt, was ansteht | `IMPLEMENTED` | Bedarf 108, `cable#758` — „users ask to be TOLD something rather than to go and check", und daraus folgt der ganze Entwurf: `lib/actionItems.ts` SAMMELT EIN und leitet nichts ab. Sechs Quellen, alle vorhanden (`overdueSubhire`, `overdueCheckouts`, `labourFindings`, `receiptChain`, `assessCosts`, `bookingConflicts`). **Fälligkeit wird nie erfunden:** `when` steht nur da, wo die Quelle ein Datum hat — auf einer nach Dringlichkeit sortierten Liste wäre ein erfundener Termin der teuerste Fehler, er schöbe die echten Überfälligkeiten nach unten. Welcher Befund eine Aufgabe IST, entscheiden vollständige `Record`s über die Befundarten: eine neue Art upstream ergibt einen Compile-Fehler statt stillen Durchrutschens. Das Abzeichen in der Statuszeile erscheint nur, wenn es etwas zu sagen gibt |
| Aufnahmenamen aus dem Plan | `IMPLEMENTED` | Bedarf 100, `cable#759` — „take numbers are incremented by hand across 10-16 camera decks; automated filename building FAILS SILENTLY while the button label still renders correctly". Der zweite Satz entscheidet: gebildet wird der Name, den das Gerät tatsächlich bekäme, und geprüft gegen das, was ein Deck annimmt — doppelter Name (zwei Decks schreiben dieselbe Datei), abgelehntes Zeichen, zu lang, keine Aufzeichnung. Eine Take-Nummer je PROJEKT und am Projekt gespeichert; eine je Rechner ergäbe auf zwei Rechnern zwei Dateinamen. Erlaubnis-Liste statt Verbotsliste, weil eine Verbotsliste immer unvollständig ist. Gebaut auf `buildHandoverManifest` statt auf einer zweiten Traversierung des Kabelgraphen (ADR-001). **Nicht gebaut:** Push an die Decks und Zustands-Rückmeldung |
| Lesen viele, schreiben einer | `IMPLEMENTED` | Bedarf 109, `cable#759` — „I do not want some one else to be able to ALTER the cue sheet once we are on site" (`ontime#1547`). Der LAN-Server hatte die Lese-Hälfte und drei Schreibwege, die immer offen waren — nicht aus Entscheidung, sondern weil die Routen existieren. `writeMode` mit Vorgabe `read-only`, `writeAllowed()` an EINER Stelle und VOR dem Lesen des Bodys (wer später prüft, hat den Body schon angenommen), 403 statt 401 (das Token war ja richtig), und alles ausser `contribute` ist `read-only` — im Server wie im Handy. `/share-info.json` nennt den Modus, das Handy blendet die Schreib-Bedienung aus und sagt, dass Häkchen lokal bleiben |
| Ausgabe rückwirkend, Termin verschiebbar | `IMPLEMENTED` | Bedarf 98, `cable#759` — „Ability to create a booking STARTING IN PAST" und „edit bookings end date WHILE IT'S GOING ON". Der Vorgang auf der Platte entsteht nach dem Vorgang in der Halle; der Store las bisher immer die Uhr. Rückwirkend ohne Frist nach hinten (eine Grenze wäre eine Vermutung darüber, wie spät jemand nachträgt), aber `in-the-future` als Befund: das wäre eine Reservierung, und ein Vorgang, der Material als draussen führt, macht die Lagerdeckung falsch. Verschieben in BEIDE Richtungen, und der alte Termin bleibt in `extensions` stehen — beim Sub-Hire hängt daran die Rechnung des Lieferanten. Die Vokabel „Load out/Load in" wurde mit Begründung nicht übernommen; „never require a scanner with no pick-from-list fallback" war erfüllt und wird jetzt geprüft statt behauptet |
| Presets ueberschreiben nicht still | `IMPLEMENTED` | Bedarf 96, `cable#721` — „Applying a preset must never silently replace an existing grid; presets must append or prompt and show the resulting row count" (Cryde/musicall#798, geschrieben NACH einem Datenverlust-Fehler). Genau das passierte in `applyTechnique`: wer Mikrofone auf drei Toms gesetzt hatte und auf eine kleinere Technik wechselte, verlor sie wortlos. Jetzt ein Bericht und eine Rueckfrage mit der Zahl DANACH; gemeldet wird nur, was ein MENSCH eingetragen hat. Die uebrigen Import-Wege erfuellen die Regel bereits (`mergeById`/`mergeDefined`, ADR-005 Regel 2; Bedarf 65) |
| Tally fuer entfernte Beitragende | `IMPLEMENTED` | Bedarf 86, `tally-pi#10` — „remote guests have no reliable way to know they are live" (vdo.ninja#569/#654, beide seit Jahren offen). **Die Erreichbarkeit wird bewusst NICHT gebaut** — ein Relay waere ein Cloud-Dienst und widerspraeche der Zusage des Repos; das steht jetzt im README statt offenzubleiben. Gebaut ist, dass die Seite nicht LUEGT: der Herzschlag war ein SSE-Kommentar und damit fuer `onmessage` unsichtbar (eine halboffene NAT-Verbindung liess den letzten Zustand fuer immer stehen), es gab keinen Wachhund, `offline` sah aus wie `safe`, und ein leerer Zustand fiel auf die beruhigende Antwort zurueck. Dazu der hoerbare Alarm aus #654 — und die Seite sagt, ob er wirklich scharf ist |
| Die Aufzeichnung fuer das Archiv | `IMPLEMENTED` | Bedarf 90, `cable#722` — „a dropped or throttled upload degrades the local recording" (obsproject/obs-studio#13147), und die Folge steht im Befund und nicht nur in der Ursache: das Material muss VERWORFEN werden. Der Plan kannte die Ausspielziele, aber nicht die Frage, WORAUF mitgeschnitten wird — also konnte er auch nicht sagen, dass beides an derselben Maschine haengt. `ArchiveRecording` traegt drei Antworten statt eines Hakens: `device` (dieses Geraet), `none-by-choice` (bewusst keine — eine Entscheidung, kein Versaeumnis) und `not-stated` (nicht gefragt; das ist selbst ein Befund und faellt nicht als Leere durch). `assessArchive` nennt vier Faelle, zwei davon aus dem KABELGRAPH abgeleitet und nicht gespeichert: `shares-encoder` (Recorder und Ausspielung sind dasselbe Geraet) und `fed-by-encoder` (der Recorder haengt am Encoder — `reachesForward`, hoechstens vier Spruenge). **Ob der Encoder tatsaechlich ueberlaeuft, sagt der Plan nicht** — das ist eine Messung an der Maschine, und eine geratene Antwort saehe wie eine aus |
| Der Multicast-Adressplan | `IMPLEMENTED` | Bedarf 72, `cable#723` — „Generate the ST 2110 multicast address plan with the standard's and the fabric's rules built in". `collectFlows` leitet die Fluss-Tabelle aus dem Kabelgraph ab, EIN Fluss je SENDE-PORT: fuenf Empfaenger an einer Kamera abonnieren eine Gruppe, nicht fuenf — genau da faellt eine Excel-Tabelle mit einer Zeile je Kabel auseinander. Vier Essenzen, bewusst feiner als `EssenceFamily` im Zeit-Plan (dort geht es um den Takt, hier um Gruppen; ANC ist eine eigene). Fuenf Befunde, darunter die beiden aus den Belegen: `pair-collision` (Adresse und Port zusammen eindeutig, WATCHOUT) und **`mac-alias`** (32 Gruppen auf einer L2-Adresse, Arista) — der Befund nennt beide Adressen UND die geteilte MAC, sonst liest er sich wie ein Fehlalarm, und er nennt die Folge (die Karte des Empfaengers filtert auf L2 und nimmt beide Stroeme an) ohne einen Ausfall zu behaupten. **Die Vergabe vergibt nur NACH** — eine verteilte Adresse steht im Sender, im Empfaenger, im Switch-Filter und auf einem ausgedruckten Blatt. Zwei Lade-Pfad-Fehler mitrepariert: `interfaceIsEmpty` zaehlte die ROLLE nicht mit, also verschwand eine Medien-Schnittstelle, die (waehrend der Planung) nur ihre Rolle trug |
| Das Sicherheitsnetz | `IMPLEMENTED` | Bedarf 89, `cable#724` — „the safety net's characteristic failure is parking the show on a slate while the stream is fine". Der teure Fehler ist ein FALSCH-POSITIV: ein Netz, das nicht ausloest, kostet eine Show; eines, das grundlos ausloest, auch — und niemand sucht den Fehler beim Netz, weil das Netz ja „funktioniert". Beide Belege (NOALBS #178, #119) sind von dieser Sorte. Sechs Befunde: der Namensabgleich gegen die Szenen des Encoders (die Bedarfs-Datenbank nennt ihn selbst „a cheap win", geprueft werden ALLE DREI Szenen), `scenes-unknown` (ohne Szenenliste laeuft er nicht — und das wird GESAGT, ein stiller Durchlauf saehe aus wie „geprueft"), `no-return-path` (#119: einmal umschalten und nie zurueck), `stats-loopback` (#178: `localhost` ist keine Adresse, sondern eine Annahme ueber die Maschine — der Plan kennt beide), und zwei Schwellen-Pruefungen gegen die geplante Bitrate. Das JSON-Geruest ist AUSDRUECKLICH keine einspielbare NOALBS-Datei, und der Hinweis steht IN der Datei; ohne Stream-Key und ohne Ingest-URL |
| Vorlagen mit Haus | `IMPLEMENTED` | Bedarf 91, `cable#725` — „the most valuable document in the role is the one least likely to exist". Die Vorlage gab es schon; sie trug das FALSCHE mit. `saveUserTemplate` strippte die Auftrags-Identitaet (Kunde, Logos, Rentman) und liess alles andere stehen — inzwischen also auch die Genehmigungen EINES Hauses (`venueAnswers`, Bedarf 85), die Multicast-Adressen EINER Show (Bedarf 72), das Schluessel-Haekchen EINES Rechners (Initiative 9) und die Revisionen einer fremden Show. Eine neutrale Vorlage, die die Genehmigungen der Halle A mitbringt, ist schlimmer als eine ohne: die Zeilen sehen aus wie Auskunft. `stripForTemplate` kennt jetzt zwei Umfaenge, und `saveUserTemplate` nimmt den als PFLICHT-PARAMETER OHNE DEFAULT (wie `credentials` bei `syncSharedLibrary`) — der Compiler fand prompt den einen Aufrufer. Gefragt wird nur, wenn etwas dranhaengt. Der Multicast-POOL bleibt (das Schema ist das Wiederverwendbare), die Vergaben gehen. `templateCarryReport` sagt vor dem Laden, ob die Antworten hier gelten, und benutzt dafuer `sameVenue` aus `venueAnswers` — zwei Wahrheiten darueber, was „dasselbe Haus" heisst, waeren hier besonders teuer. **Der geteilte Ordner fehlt bewusst**: Vorlagen liegen weiter im localStorage, `syncSharedLibrary` deckt Geraete und Gruppen ab, und das dort zu ergaenzen ist ein eigenes Inkrement mit eigenem Sperr- und Merge-Verhalten |
| Kiste und Netz-Identitaet | `IMPLEMENTED` | Bedarf 78, `cable#726` — „a last-minute substitution of one identical stagebox for another is invisible to both the IP plan and the Dante subscription set, and only surfaces as a fault during rehearsal". Zwei baugleiche Stageboxen sind IM PLAN dasselbe Kaestchen, IM LAGER zwei Einheiten und IM NETZ zwei verschiedene Geraete — jede mit eigenem eingebranntem Namen und eigener MAC. `EquipmentItem.inventoryUnitId` (neu) sagt, WELCHE Kiste den Platz fuellt; `assetIdentity.ts` meldet sechs Faelle, aber NUR an Plaetzen mit Netz-Identitaet (IP, MAC, Geraete-Name bei vorhandener Schnittstelle, PTP-Domaene) — ein Stativ hat keinen eingebrannten Namen, und es mitzumelden hiesse, den halben Plan zu melden. `serial-mismatch` faengt den Tausch an den beiden Seriennummern und behauptet ausdruecklich NICHT, welche stimmt: der Plan weiss nicht, was im Rack steht, und ein Widerspruch zwischen zwei Listen laesst sich am Schreibtisch aufloesen, eine Behauptung ueber die Wirklichkeit nicht. Bestand und Ausgabescheine werden nur GELESEN. Zwei bestehende Guards forderten das neue Feld sofort ein und hatten beide recht: `modelFields` (Instanz — in einem Template zeigten zwei Plaetze auf dieselbe Kiste) und `planDiff` (substantiell — wer es aendert, hat getauscht) |
| Grundlage der Uebergabe | `IMPLEMENTED` | Bedarf 84, `cable#727` — „next year the same event is re-planned from the QUOTE, not from what was actually built". **Die Datei gab es schon**: die `.avplan` ist verlustfrei (ADR-005), und `handoverPackage.ts` baut das Closeout-Paket. Was fehlte, ist der Satz aus dem Bedarf — das Naechste-Jahr-Problem ist nicht die fehlende Datei, sondern dass die vorhandene den ANGEBOTSSTAND traegt und aussieht wie der Bauzustand. `jobHandover.ts` nennt drei Zustaende: `as-quoted`, `drifted` (ein As-Built liegt vor und der Plan ist seither substantiell weitergezogen -- der teuerste Fall, weil das Blatt das Wort traegt und nicht mehr stimmt) und `as-built`. „Substantiell" kommt aus `planDiff` und wird NICHT selbst gerechnet: zwei Vorstellungen davon faerbten das eine das Blatt, das andere den Diff. `ProjectTemplate.basis` wird BEIM SCHREIBEN eingefroren (wie `VenueAnswer.venue`) und aus dem QUELL-Projekt gelesen — die gestrippte Kopie hat die Revisionen bereits verloren, und aus ihr waere jede Vorlage „wie geplant". **Nicht im Dokument-Register**: der bestehende Guard „keine Ableitung haengt an der Revisionsliste" hat den ersten Wurf gefangen, zu Recht — der Revisions-Vergleich spannt Snapshots ohne diese Liste auf. Es steht in `UNJUDGEABLE_DOCUMENTS`, mit dem Grund |
| Notizen aus der Probe | `IMPLEMENTED` | Bedarf 71, `light#72` — „an open request asks for the opposite: store notes locally rather than in the show file" (cue-note #11/#7). Die LEUCHTEN-Haelfte war gebaut (`focusNote` + `focused`, Fokus-Liste, Plan-Marker); gefehlt haben genau die drei Punkte, an denen der Beleg haengt: MEHRERE Notizen je Ziel (`focusNote` ist ein Feld -- der zweite Zuruf ueberschrieb den ersten), Notizen an der POSITION („Traverse 2 haengt 20 cm zu tief" hat keine Leuchte) und ein AUTOR mit ZEITPUNKT. `focusNote` bleibt daneben stehen: es beschreibt den Fokus und gehoert zur Leuchte wie ihr Aim, eine Arbeits-Notiz ist ein Vorgang. Eine Notiz verschwindet NIE still -- wer eine Leuchte loescht, loescht die Beobachtung nicht (`staleNotes`, im Dialog unter „Ziel entfernt"). Der Kern des Belegs steht als GUARD im CI (`work-notes-check.ts`): am Ergebnis geprueft, dass weder MVR-Export noch Venue-Austausch den Notiztext tragen, plus die Gegenprobe, dass er in der eigenen Liste STEHT |
...
| Dev-Einbettung der Planer | `COMPLETE` | Fallback-Ports 4181-4183, und kein Planer hoerte dort — der dokumentierte Dev-Weg traf nie einen laufenden Planer (B-17). Behoben mit `cable#763`, `multicam#106`, `light#90`, `suite#155`: die drei Planer pinnen genau die Ports, die Shell und README seit jeher nennen, jeweils mit `strictPort: true`. Das Pinnen allein haette nicht gereicht — ohne die Angabe rueckt Vite bei besetztem Port still weiter, und genau dieses Weiterruecken war die zweite Haelfte des Defekts |
| Rückweg Suite → upstream | `IMPLEMENTED` | der Drift-Bericht **nennt** die suite-ahead-Dateien (`suite#64`) und sagt seit 2026-09-10 auch, ob jemand draufgeschaut hat: die Spalte `gesichtet` vergleicht die heutige Zeilenzahl mit der in `GESICHTET` (`scripts/planner-drift.mjs`) eingefrorenen und schreibt `ja` / `gewachsen (N statt M)` / `offen`. Alle **acht** Dateien durchgegangen: acht bewusste Überlagerungen (Lexware, Shell-Einbettung), **null** Rückportierungen. Die Bewertung Overlay/Fix bleibt Handarbeit — gemessene Trefferquote roher Kandidaten: 3 von 9; der Haken sagt „angesehen", nicht „unbedenklich" |
...
| Kette | Status | Beleg |
| --- | --- | --- |
| Plan → Tally-Pi | `COMPLETE` | Datenvertrag `COMPLETE` und getestet. **Aus der Suite laeuft der Transport** seit `suite#99`: `apps/shell/electron/tallyPush.cjs` schickt die Karte per `POST /tally-config` an den Pi, mit Abgleich davor — aus dem Hauptprozess und nicht per `fetch` aus dem Renderer, weil der Pi-Server weder CORS-Kopfzeilen noch einen OPTIONS-Zweig hat. **Aus dem `cable-planner` selbst bleibt es ein Download** (`ExportDialog.tsx`), den jemand nach `/opt/pi-guide/tally.json` kopiert — **seit `cable#797` liefert auch der Planer selbst direkt** (`src/main/services/tallyPushService.ts`, `src/main/ipc/tallyIpc.ts`, `tests/tallyDirektweg.test.ts`). E-7 ist damit entschieden — **beides**: die Datei bleibt der Vorgabeweg, der Direktweg steht daneben. B-6 ist gebaut. **Nachgemessen 2026-09-09**; die Zeile nannte E-7 bis dahin offen |
...
| Intercom ↔ Suite | `PARTIAL` | **Nachgemessen 2026-09-18; die Zeile war an zwei Stellen veraltet.** Das gemeinsame Vokabular gibt es, und es steht inzwischen auf **Version 2** — `avplan-intercom` mit der TASTENBELEGUNG, geschrieben von `cable-planner` (`INTERCOM_FORMAT_VERSION = 2`) und gelesen von `Broadcast-intercom` (`INTERCOM_PLAN_VERSION = 2`); beide Seiten in derselben Sitzung gehoben, damit wer nur eine aktualisiert eine ehrliche Fehlermeldung bekommt statt einer halben Uebernahme. **E-2 ist beantwortet** (2026-09-08, vom Eigentuemer): neutrales Format, `GreenGoConfig` als Ausgabe-Projektion, eigener Projekt-Slot — gebaut in `cable#791`/`#792` (`types/intercomPlan.ts`), `greengoConfig` wird beim Laden entfernt statt danebengelegt. **Was bleibt, ist genau eine Sache, und sie ist absichtlich nicht gebaut:** `KNOWN_DOMAIN_SLOTS` in `avplan.ts` kennt weiter nur `cameras`, `lighting`, `cabling`. Eine Intercom-Domaene im `.avplan`-Container faehrt heute als UNBEKANNTER Slot mit — also verlustfrei bewahrt und beim Laden benannt (ADR-005 Regel 3). Sie zu BENENNEN hiesse, den Namen in fuenf Kopien nachzuziehen, waehrend ihn niemand schreibt und niemand liest: `Broadcast-intercom` nimmt seinen Plan als eigene Datei entgegen (B-8), nicht als Container. Der Name kommt, wenn die App den Container liest — vorher waere er eine Zusage ohne Gegenstand (B-7) |
