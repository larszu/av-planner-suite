...
| Intercom ↔ Suite | `PARTIAL` | **Das gemeinsame Vokabular gibt es** — `avplan-intercom` v1, geschrieben von `cable-planner` (`lib/intercomExchange.ts`, `cable#684`) und gelesen von `Broadcast-intercom` (`packages/shared/src/index.ts`, `#9`/`#14`: `readIntercomPlan` mit `PlanSkipped`, zusammengefuehrt statt ersetzt). Was fehlt, ist der SLOT: `KNOWN_DOMAIN_SLOTS` in `avplan.ts` kennt `cameras`, `lighting`, `cabling` — Intercom faehrt als eigene Datei neben dem Plan statt in ihm. Ob das Vokabular ein gemeinsames Paket wird oder `GreenGoConfig` die Wahrheit bleibt, ist die offene Eigentuemer-Frage E-2 (B-7) |
