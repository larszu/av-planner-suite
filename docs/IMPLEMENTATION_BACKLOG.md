...
| E-21 | Wer gewinnt, wenn **MultiCam und Licht denselben Raum** ändern? | B-39.1 — der Seed trägt den Raum bisher nur hin; ein Rückweg ohne diese Regel überschriebe die Änderung des jeweils anderen Planers. **Hochgestuft 2026-09-07: blockierend.** Die Entscheidung zu E-22 („alle lesen und schreiben die Zuordnung, `@avplan` reicht sie weiter") macht aus derselben Frage die Vorbedingung von Bedarf 49. Sie ist nicht mehr nur die Raum-Frage aus B-39.1, sondern die Regel für jedes Feld, das mehr als ein Planer schreibt |
...
| E-23 | Spricht die Suite eine **Show-Control-Sprache** — sendet oder liest sie Ereignisse, auf die eine Bedienoberfläche reagiert? | Bedarf 35 (P1) — die Bedarfs-Datenbank stellt die Maßnahme ausdrücklich unter diese Bedingung: „**If** the suite emits or consumes show-control events, transmission state belongs in that vocabulary; **do NOT build a live monitoring dashboard**, which would make the suite responsible for a false all-clear." Nachgesehen 2026-09-07: `cable-planner` sendet keine solchen Ereignisse. Es gibt die Tally-Karte an `tally-pi` (`lib/tallyMap.ts`) und Companion-Tasten für Intercom (`Broadcast-intercom#10`) — beides Konfiguration, die vorher hinausgeht, kein Ereignis zur Laufzeit. **WEITER RECHERCHIERT 2026-09-07, damit die Entscheidung auf Tatsachen fällt:** eine Bedienoberfläche spricht heute nicht EINE Sprache, sondern nimmt fast jede an. Bitfocus Companion lässt sich über **OSC, TCP, UDP, HTTP, WebSocket und ArtNet** auslösen und kann zusätzlich **Ember+** mithören; seine HTTP-Schnittstelle kennt `POST /api/location/<page>/<row>/<column>/press` für einen Tastendruck und `POST /api/custom-variable/<name>/value?value=<v>` für einen Wert. Daneben stehen die allgemeinen Vokabulare OSC und MIDI Show Control. **Was das für die Entscheidung heisst:** „nur ausgehend" braucht kein neues Protokoll und keine Laufzeit-Verbindung — der Plan benennt je Ausspielziel eine Custom-Variable oder eine Tastenposition, und das ist Konfiguration derselben Sorte wie die Tally-Karte. „Beidseitig" wäre dagegen genau die Verantwortung, die die Bedarfs-Datenbank verbietet: wer Zustand anzeigt, haftet für die Entwarnung. Quellen: [Companion HTTP Remote Control](https://companion.free/user-guide/v4.2/remote-control/http-remote-control/), [Companion Settings](https://companion.free/user-guide/v4.1/config/settings/), [Satellite API](https://companion.free/for-developers/Satellite-API/), [Open Sound Control](https://en.wikipedia.org/wiki/Open_Sound_Control), [MIDI Show Control](https://en.wikipedia.org/wiki/MIDI_Show_Control) — die drei `companion.free`-Seiten liegen hinter dem Egress-Filter dieser Sitzung und sind über die Suchtreffer belegt, nicht am Volltext gelesen. **WEITER RECHERCHIERT 2026-09-07 (zweiter Durchgang), diesmal an Primärquellen — mit einer BERICHTIGUNG am eigenen Zwischenstand:** der Absatz oben legt nahe, „nur ausgehend" sei auch deshalb der Weg, weil Companion sich schlecht auslesen lässt. **Das stimmt nicht mehr.** [bitfocus/companion#2995](https://github.com/bitfocus/companion/issues/2995) beschrieb genau diese Lücke — *„There is a 'CUSTOM-VARIABLE name SET-VALUE value' command but no corresponding 'GET-VALUE'"* — und ist geschlossen durch [PR #3999](https://github.com/bitfocus/companion/pull/3999) („feat: Get custom-variable via tcp", von Julusian **am 2026-03-04 nach `main` gemergt**): `CUSTOM-VARIABLE <name> GET-VALUE` gibt es seitdem, mit `JSON.stringify` für die Typen. Beide Seiten sind also offen, lesend wie schreibend. **Was das ändert und was nicht:** es nimmt der Empfehlung ein FALSCHES Argument, nicht ihr richtiges. Die Bedingung der Bedarfs-Datenbank hängt nicht daran, was Companion kann, sondern daran, wofür die Suite haftet — *„do NOT build a live monitoring dashboard, which would make the suite responsible for a false all-clear"*. Dass Auslesen jetzt technisch leicht ist, macht es nicht verantwortbar; es macht die Versuchung nur grösser. Die Empfehlung „nur ausgehend, als Konfiguration" bleibt, aber jetzt aus dem Grund, der trägt. **Zwei weitere Tatsachen aus diesem Durchgang:** (1) Companions TCP-/HTTP-/OSC-/UDP-Schnittstellen sind **opt-in** und laufen auf frei wählbaren Ports — ein Plan, der eine Companion-Adresse benennt, muss deshalb mitschreiben, dass sie eingeschaltet sein muss, sonst zeigt er einen Weg, den es beim Kunden nicht gibt. (2) OSC wird auf der Broadcast-Seite gerade zur Verkehrssprache: SSL öffnet System T mit **V4.3 zur IBC 2026** für OSC **ein- und ausgehend** im Event Manager, ausdrücklich neben GPIO, Ember+ und MIDI ([rAVe](https://ravepubs.com/solid-state-logic-to-present-updated-system-t-platform-at-ibc-2026/), [Mixdown](https://mixdownmag.com.au/news/third-party-control-comes-to-ssls-broadcast-console/)). Fällt die Entscheidung auf „ausgehend", ist OSC damit die Vokabel mit der breitesten Anschlussfähigkeit — und sie kostet nichts, weil der Plan sie nur BENENNT und nicht spricht |
| E-24 | Bindet die Suite **CuePilot** und **LiveEdit** an — und wenn ja, in welcher Richtung? | **Nutzerwunsch vom 2026-09-07** („Sowas wie CuePilot & LiveEdit müsste man auch noch einbinden"). Recherchiert am selben Tag, damit die Frage auf Tatsachen steht. **Was die beiden sind:** *CuePilot* ist dedizierte Software für Live-TV-Produktion (von Per Zachariassen; Referenzen bis Eurovision). Pro Akt gibt es eine **Zeitachse mit Cues**; in der Vorproduktion wird jeder Bildmischer-Schnitt und jeder Cue geplant, in der Show spielt CuePilot sie ab: es steuert den Bildmischer, spielt in die **CueApp** (iOS/Android, jede Abteilung sieht ihre eigenen Cues mit Countdown) und andere Systeme über **OSC, GPI und MIDI**; Lichtpult, Bildmischer, Medienserver und Cue-Geräte liegen auf EINER Zeitachse. *LiveEdit* (liveedit.app) ist ein eigenes Produkt, kein CuePilot-Modul: „combine all your cues and notes in one place, and share them with your crew in real time". Dass beide Welten Daten tauschen, ist belegt: der **CuePilot Importer** für GrandMA3 importiert laut Produktbeschreibung „a LiveEdit track into your GrandMA3 showfile" — es gibt also ein **Cue-Track als Austauschgegenstand**, und mindestens ein Lichtpult liest ihn schon. **Warum das genau unsere Grenze berührt:** die Suite hat seit `suite#142` einen eingelesenen Ablauf, der über Objekt-Ids am technischen Plan hängt (Bedarf 8). CuePilot/LiveEdit führen die Ebene DARUNTER — nicht „was läuft wann", sondern „welcher Schnitt, welcher Cue, auf welchem Frame". **Die Frage ist deshalb die Richtung, und sie hat drei Antworten mit sehr verschiedenen Kosten:** (a) **nur lesen** — einen Cue-Track einlesen und gegen den technischen Plan halten (welche Kamera, welches Kabel, welche Funkstrecke hängt an diesem Cue); das ist dieselbe Bauform wie E-18 und braucht kein Protokoll. (b) **nur schreiben** — aus dem Plan einen Track erzeugen, den CuePilot oder GrandMA3 importiert; das setzt ein dokumentiertes Format voraus, und genau das ist noch **unbelegt**. (c) **beidseitig zur Laufzeit** — fällt unter dieselbe Bedingung wie E-23 („do NOT build a live monitoring dashboard"), also vermutlich WON'T. **Was zuerst geklärt werden muss, bevor überhaupt entschieden werden kann:** das Austauschformat. `cuepilot.com`, `liveedit.app` und `cuepilot.zendesk.com` liegen hinter dem Egress-Filter dieser Sitzung; die Angaben oben stammen aus Suchtreffern und aus der Produktseite des GrandMA3-Importers, **nicht am Volltext gelesen**. Ohne eine echte Beispieldatei oder eine Formatdoku wäre jeder Bau ein Ratespiel. Verwandt: E-23 (Show-Control-Vokabel), Bedarf 10 (Ablauf live an der Kamera), Bedarf 45 (Weg vom Ablauf-Inhalt zur Bedienoberfläche) |
