...
* **Status:** **upstream gebaut (`light#97`, 2026-09-08), Vendoring offen.** Entschieden war E-15: Vorschau-Schritt wie beim Ablauf-Import; der Drei-Wege-Dialog und das Undo für den Lager-Store sind damit erledigt, nicht aufgeschoben. Gebaut ist `src/inventory/importPreview.ts` — es rechnet je Sorte und je Modus mit **derselben** `mergeDefined`, die der Import danach benutzt, und der Guard `preview:check` hält die Vorschau gegen `mergeById` selbst. Neu gegenüber der Entscheidung ist die Kategorie `unberuehrt`: dieselben Datensätze, die `replace` löscht, bleiben bei `merge` stehen — genau darin besteht der Unterschied zwischen den beiden Antworten, und er war sonst nirgends abzulesen. Gefunden hat sie der Guard, nicht der Entwurf: die Rückrichtung („jede Id, die vorher oder nachher existiert, kommt in der Vorschau vor") war rot. **Zwei Stellen bleiben offen:** die vendorte Kopie unter `apps/light-planner/` (wartet auf den Merge von `light#97`) und **`apps/multicam-planner/src/inventory/`, wo derselbe Befund unangetastet steht** — dort ist die Frage ein `confirmDialog`, dessen `false` auch von Escape und vom Klick daneben kommt. **Das Zwischenmaß steht** (`suite#154`, 2026-09-07): Escape und Hintergrund-Klick importieren nicht mehr — es bleibt, bis die Vorschau es ablöst.
