...
* **Status:** offen — **entschieden (E-2, 2026-09-08): das neutrale Format ist die Wahrheit, gemeinsames Paket, kein vierter `.avplan`-Slot.** Was bleibt, ist Bauarbeit
...
* **Status:** offen — **entschieden (E-15, 2026-09-08): Vorschau-Schritt wie beim Ablauf-Import** — **das Zwischenmaß ist umgesetzt** (`suite#154`, 2026-09-07): Escape und Hintergrund-Klick importieren nicht mehr. Der dritte Ausgang (eigener Drei-Wege-Dialog, Vorschau-Schritt oder Undo für den Lager-Store) bleibt die Entscheidung.
...
**Drei Zeilen tragen eine Bedingung, die kein Beschluss aufhebt** und die
deshalb ausdrücklich zur Entscheidung gehört: E-24 wartet auf eine echte
Beispieldatei (das Format ist unbelegt und liegt hinter dem Egress-Filter),
E-6 auf denselben Netzzugang wie B-11, und E-23 ist ein *Verbot* mit Grund —
nur ausgehend, nie ein Laufzeit-Dashboard.
...
| ~~E-2~~ | ~~Intercom-Vokabular: gemeinsames Paket oder `GreenGoConfig` als Wahrheit?~~ | **entschieden 2026-09-08: das herstellerneutrale Format aus B-8 ist die Wahrheit, `GreenGoConfig` wird zur Projektion, und das Vokabular zieht in ein gemeinsames Paket.** `GreenGoConfig` kann die Wahrheit nicht sein: sie ist die Konfiguration EINES Herstellers, und B-8 hat das neutrale Format bereits gebaut (`cable#684`) — ein Format, das über dem Hersteller steht, danach zur Projektion eines Herstellers zu erklären, wäre eine Rückwärtsbewegung. Zwei Verbraucher gibt es schon (cable-planner und `Broadcast-intercom/packages/shared`, 437 Zeilen), also gibt es auch den Anlass für das Paket. **Kein vierter `.avplan`-Slot:** der Plan trägt die Wahrheit bereits (Rollen, Ports, Kanäle); die Intercom-Konfiguration wird daraus ABGELEITET wie die Tally-Karte. Ein eigener Slot machte aus einer Ableitung eine zweite Quelle — genau der Fehler, den ADR-001 benennt |
| ~~E-3~~ | ~~Soll der Techniker vor Ort den **Anlagen-Pincode** über die Mobile-Ansicht bekommen?~~ | **entschieden 2026-09-08: NEIN — und zwar nicht als Abwägung, sondern als Regel.** Ein Pincode ist ein Zugangsmittel; `CLAUDE.md` legt für diese Sorte fest, dass sie im OS-Credential-Store liegt und weder geloggt noch ins Projekt-File geschrieben wird. Die Mobile-Ansicht wird über einen QR-Link an jedes Telefon im WLAN ausgeliefert — sie ist der denkbar schlechteste Kanal dafür. `cable#656` hat das Leck geschlossen; es wieder zu öffnen, wäre ein Rückschritt mit Bequemlichkeits-Begründung. **Was stattdessen gebaut wird:** die Mobile-Ansicht zeigt, WEN man fragt — Ansprechpartner und Erreichbarkeit aus der Haus-Antwort (Bedarf 85, `lib/venueAnswers.ts`). Das löst den Arbeitsablauf, ohne das Geheimnis zu verteilen |
...
| ~~E-23~~ | ~~Spricht die Suite eine **Show-Control-Sprache**?~~ | **entschieden 2026-09-08: NUR AUSGEHEND, und zwar als Konfiguration statt als Laufzeit-Verbindung; die Vokabel ist OSC.** Die Bedarfs-Datenbank stellt die Massnahme unter eine Bedingung, die sich nicht wegrecherchieren lässt: *„do NOT build a live monitoring dashboard, which would make the suite responsible for a false all-clear"*. Wer Zustand anzeigt, haftet für die Entwarnung — und dieses Repo hat keinen Weg, eine Entwarnung zu verifizieren. Dass Companion sich inzwischen auch AUSLESEN lässt (`CUSTOM-VARIABLE … GET-VALUE`, gemergt 2026-03-04), ändert daran nichts: es nimmt der Empfehlung ein falsches Argument, nicht ihr richtiges. **Was gebaut wird:** der Plan BENENNT je Ausspielziel eine OSC-Adresse bzw. Companion-Position und druckt sie — dieselbe Sorte Konfiguration wie die Tally-Karte, ohne Verbindung zur Laufzeit. OSC, weil es die breiteste Anschlussfähigkeit hat und nichts kostet, solange der Plan es nur benennt. **Mit der Auflage aus der Recherche:** wo eine Companion-Adresse steht, schreibt das Blatt dazu, dass deren Schnittstelle opt-in ist — sonst zeigt der Plan einen Weg, den es beim Kunden nicht gibt |
