...
| E-22 | Wo wohnt die Zuordnung **Kamera → Bedienfeld → Multiviewer → Tally**? | Bedarf 49 — die Bedarfs-Datenbank verlangt sie „once in the project file", damit `tally-pi`, `cable-planner` und `sony-camera-bridge` DIESELBE lesen. Heute gibt es zwei Hälften und keine Naht: `multicam-planner` führt Kamera→Bedienfeld als Klartext (`PaintState.panel`, Bedarf 50), `cable-planner` führt Quelle→Multiviewer→Tally im Router-Modell (`lib/tallyMap.ts`). Beide zu Recht — es sind die Pläne zweier Gewerke. Die Frage ist, welcher der beiden die gemeinsame Zuordnung TRÄGT (und der andere sie liest), oder ob sie in ein drittes, geteiltes Objekt wandert. Ohne diese Antwort wäre jede Implementierung die dritte Kopie statt der einen Wahrheit — genau der Zustand, den der Bedarf abschafft. Verwandt mit E-4 (wo der As-built-Zustand wohnt) und E-11 (gemeinsamer Id-Raum) |
| E-23 | Spricht die Suite eine **Show-Control-Sprache** — sendet oder liest sie Ereignisse, auf die eine Bedienoberfläche reagiert? | Bedarf 35 (P1) — die Bedarfs-Datenbank stellt die Maßnahme ausdrücklich unter diese Bedingung: „**If** the suite emits or consumes show-control events, transmission state belongs in that vocabulary; **do NOT build a live monitoring dashboard**, which would make the suite responsible for a false all-clear." Nachgesehen 2026-09-07: `cable-planner` sendet keine solchen Ereignisse. Es gibt die Tally-Karte an `tally-pi` (`lib/tallyMap.ts`) und Companion-Tasten für Intercom (`Broadcast-intercom#10`) — beides Konfiguration, die vorher hinausgeht, kein Ereignis zur Laufzeit. Eine Vokabular dafür zu erfinden, wäre eine neue Produktschicht und keine Zeile im Ablieferungs-Modell. Verwandt mit E-18 (Zeitachse) und E-22 (die eine Zuordnung): dieselbe Frage nach dem Zuschnitt, dreimal an verschiedenen Kanten |
