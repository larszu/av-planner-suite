...
/**
 * B-53 — DIE VERSION IST KEINE ABHAENGIGKEIT.
 *
 * `package.json` bleibt zeilenweise draussen, und das ist richtig: Monorepo
 * und Standalone haben zwangslaeufig verschiedene Abhaengigkeiten und
 * Skripte. Der Ausschluss schuettete aber ein Feld mit aus, das mit dem
 * Monorepo-Zuschnitt nichts zu tun hat — `version`.
 *
 * GEMESSEN am 2026-09-09: `apps/cable-planner` stand auf 8.3.1, upstream auf
 * 9.0.1; `apps/multicam-planner` auf 4.3.0 gegen 4.3.3. Und weil
 * `update-doc-stats.mjs` die Version von dort liest, schrieben
 * `docs/architecture.md`, `docs/app-structure.html` und `docs/comparison.html`
 * in der Suite „Stand: v8.3.1" ueber einen Quelltext, der v9.0.1 IST. Die
 * Modul-Zahl daneben stimmte. Das ist die unangenehmere Sorte falsch: eine
 * Zahl, die sich richtig anfuehlt, in einer Zeile, deren Nachbarn stimmen.
 *
 * Deshalb wird genau dieses eine Feld verglichen — benannt, nicht durch
 * Aufheben des Ausschlusses.
 */
const versionOf = (pfad) => {
  try {
    const v = JSON.parse(readFileSync(pfad, 'utf8')).version
...
for (const r of results) {
  ...
  summary[r.app] = {
    ...
    // B-53. In der Baseline steht die Version des Vendoring-Zeitpunkts; der
    // Check unten braucht sie, um „hier falsch abgeschrieben" von „upstream
    // hat inzwischen erhoeht" zu unterscheiden.
    version: versionOf(join(ROOT, 'apps', r.app, 'package.json')),
    upstreamVersion: r.missingUpstream
      ? null
      : versionOf(join(upstreamRoot, r.app, 'package.json')),
  }
}
...
for (const app of APPS) {
  const s = summary[app]
  const sha = s.missingUpstream ? 'upstream checkout not found' : upstreamSha(app)
  // B-53: die Version steht daneben, weil die Doku der Kopie sie abschreibt.
  const ver =
    s.missingUpstream || s.version === s.upstreamVersion
      ? s.version ?? '?'
      : `${s.version ?? '?'} — upstream ${s.upstreamVersion ?? '?'} (ABWEICHEND)`
  lines.push(`- \`${app}\` @ ${sha} · v${ver}`)
}
lines.push('')
...
if (has('check')) {
  ...
  for (const app of APPS) {
    ...
    // B-53 — die Version der vendorierten Kopie.
    //
    // Hier und nicht weiter oben: der `continue` beim bewegten Upstream soll
    // auch diese Pruefung ueberspringen, und aus demselben Grund. Hat upstream
    // seit der Baseline erhoeht, ist die abweichende Version kein Fehler
    // dieses Repos, sondern der faellige Nachzug — sie wird gemeldet
    // (Bericht-Kopfzeile) und bricht keinen fremden Suite-PR.
    //
    // Steht der Upstream dagegen genau da, wo die Baseline ihn festhielt,
    // dann IST die Abweichung hier entstanden: jemand hat vendoriert und die
    // Version stehen lassen. Und dann behauptet die Doku dieser Kopie eine
    // Version, die ihr Quelltext nicht hat.
    if (now.upstreamVersion && now.version !== now.upstreamVersion) {
      console.error(
        `FEHLER ${app}: vendorierte Version ${now.version} != upstream ` +
        `${now.upstreamVersion} (upstream unveraendert seit der Baseline). ` +
        `Das Feld \`version\` in apps/${app}/package.json mitziehen und ` +
        `\`npm run docs:stats\` IN der Kopie laufen lassen.`,
      )
      failed = true
    }
